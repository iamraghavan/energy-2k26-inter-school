...
export function Score({ match, compact=false }: { match: Match; compact?: boolean }) {
  ...
  if (match.sport==='cricket') {const batting=s.innings===2?match.team_b:match.team_a,opponent=s.innings===2?match.team_a:match.team_b,ballsLeft=s.target?Math.max(0,120-(s.balls||0)):0,needed=s.target?Math.max(0,s.target-(s.runs||0)):0;return <div key={revision} className="cricket-score score-change"><div className="cricket-teams"><b>{batting.short_name}</b><span>vs {opponent.short_name} · INNINGS {s.innings||1}</span></div><strong>{s.runs||0}<small>/{s.wickets||0}</small></strong><span>{Math.floor((s.balls||0)/6)}.{(s.balls||0)%6} OVERS</span>{s.target&&<><em>Target {s.target}</em><small className="chase-line">Need {needed} from {ballsLeft} balls</small></>}</div>}
