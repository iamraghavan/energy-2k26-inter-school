...
export function Admin(){
 ...
 const manageableMatches = matches.filter(m => m.status === 'scheduled' || m.status === 'completed').sort((a,b) => a.status.localeCompare(b.status) || +new Date(b.updated_at) - +new Date(a.updated_at))

 const deleteMatch = async (matchId: string) => {
   const match = matches.find(item => item.id === matchId)
   const label = match?.status === 'scheduled' ? 'scheduled fixture' : 'completed match'
 ...
 return <><Nav/><main className="portal"><div className="portal-head"><div><span className="kicker">TOURNAMENT CONTROL</span><h1>Match registration</h1><p>Register teams on the ground and schedule the match in one step.</p></div><div className="admin-stats"><span><CalendarPlus/>{matches.length} fixtures</span><span><Users/>{teams.length} teams</span></div></div><section className="admin-grid"><QuickSchedule teams={teams} onCreated={() => { loadTeams(); refresh(); }}/><aside className="workflow"><h2>Optimized workflow</h2><div><b>01</b><span><strong>Register</strong>Type or select both teams</span></div><i/><div><b>02</b><span><strong>Start</strong>Confirm teams on ground</span></div><i/><div><b>03</b><span><strong>Finish</strong>Score locks as view-only</span></div></aside></section><section className="admin-announcements"><div className="announcement-admin"><h2>Manage Matches ({manageableMatches.length})</h2><p style={{color:'#789087',marginBottom:14}}>Delete scheduled fixtures or clear finished matches. Live matches stay protected.</p><div className="announcement-list">{manageableMatches.length ? manageableMatches.map(m => <article key={m.id} style={{gridTemplateColumns:'1fr auto'}}><p><b>{sportName(m.sport)}</b> ({m.gender}) — <span>{m.team_a.short_name} vs {m.team_b.short_name}</span> <small style={{color:'#888',marginLeft:8}}>{m.venue} · {m.status}</small></p><div><button className="delete" title={`Delete ${m.status} match`} onClick={() => deleteMatch(m.id)} disabled={busy}><Trash2 size={16}/></button></div></article>) : <p className="quiet">No scheduled or completed matches.</p>}</div></div></section><div className="admin-announcements"><AnnouncementManager/></div></main></>
}
