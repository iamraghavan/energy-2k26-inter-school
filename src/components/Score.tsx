...
export function Score({ match, compact=false }: { match: Match; compact?: boolean }) {
  ...
  if (match.sport==='cricket') {const batting=s.battingTeam==='b'?match.team_b:match.team_a,opponent=s.battingTeam==='b'?match.team_a:match.team_b,ballsLeft=s.maxBalls?Math.max(0,s.maxBalls-(s.balls||0)):undefined,needed=s.target?Math.max(0,s.target-(s.runs||0)):0;return <div key={revision} className="cricket-score score-change"><div className="cricket-teams"><b>{batting.short_name}</b><span>vs {opponent.short_name} · {s.innings===2?'2ND':'1ST'} INNINGS</span></div><strong>{s.runs||0}<small>/{s.wickets||0}</small></strong><span>{Math.floor((s.balls||0)/6)}.{(s.balls||0)%6} OVERS</span>{s.innings===2&&s.target&&<><em>Target {s.target}</em><small className="chase-line">Need {needed}{ballsLeft!==undefined?` from ${ballsLeft} balls`:''}</small></>}{s.innings===2&&s.innings1Runs!==undefined&&<small className="first-innings-line">1st innings · {opponent.short_name} {s.innings1Runs}/{s.innings1Wickets||0}</small>}</div>}
  if (match.sport==='chess') return <div key={revision} className="chess-score score-change"><strong>{s.result || 'In progress'}</strong><span>{s.board || 'Board 1'}</span></div>
