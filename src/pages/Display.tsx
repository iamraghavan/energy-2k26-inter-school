...
export function Display(){
  ...
  return <main className="display-page">
    ...
    <section className="display-grid">
      ...
      <aside className={`live-panel live-count-${live.length}`}><div className="section-heading"><span><i/>LIVE NOW</span><b>{live.length} MATCH{live.length===1?'':'ES'}</b></div><div className="live-list">{others.length?others.map((m,index)=><div className="mini-match" key={m.id}><div className="mini-card-number">{String(index+2).padStart(2,'0')}</div><div className="mini-meta"><b>{sportName(m.sport)} <em>{genderLabel(m.gender)}</em></b><span><MapPin/> {m.venue}</span></div><Score match={m} compact/></div>):<p className="quiet">Other live matches will appear here.</p>}</div></aside>
